"""
PPO_129 — Continue PPO_127 (scale=0.10) from 25M to 50M.

Part of Experiment 34 scale sweep. PPO_127 (high scale=0.10) showed delayed
phase transition (MULTIPLE_SCRIPTS at 22M vs 14M for baseline) and lower peak
scores (stoch best=105 vs 216 for baseline at 25M). Continuing to 50M tests
whether high scale prevents the oscillation or just shifts it later.

Design:
  - CONTINUATION from ./models/PPO_127/final_model.zip (25M steps)
  - Same ProximityRewardWrapper(scale=0.10, max_distance=80)
...
RESUME_FROM = "./models/PPO_127/final_model.zip"
...
if __name__ == "__main__":
    print(f"{RUN_NAME} — Continue PPO_127 (HIGH scale=0.10) 25M -> 50M")
    ...
    print(f"Loading PPO_127 from {RESUME_FROM}...")
