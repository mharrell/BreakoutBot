"""
BeamRider Baseline Tracked — Full Memorization Monitoring

Re-run of the original BeamRider probe (SEED=202, EpisodicLifeEnv, MULTIPLE_SCRIPTS
at 10M) but WITH MemorizationCheckCallback at 1M intervals so we can see the
trajectory: does it start SINGLE_SCRIPT and break out, or MULTIPLE_SCRIPTS from
the start?

Design:
  - Training: BeamRider WITH EpisodicLifeEnv (hard failure, 1 life/segment)
  - Eval/Check: Same (EpisodicLifeEnv)
  - Standard PPO: NatureCNN, ent_coef=0.006
  - 9 actions (default ALE beam_rider action space)
  - Target: 10M steps

Comparison:
  Multi-life variant (BEAMRIDER_MULTILIFE, SEED=205, no EpisodicLifeEnv):
    predicted SINGLE_SCRIPT — soft failure removes reactive requirement.
  This tracked baseline (SEED=206, EpisodicLifeEnv):
    predicted MULTIPLE_SCRIPTS — replicates original SEED=202 result with
    checkpoint-by-checkpoint trajectory data.
"""
import os
import numpy as np
import glob
...
def make_training_env():
    """BeamRider WITH EpisodicLifeEnv — hard failure, 1 life per segment."""
...
if __name__ == "__main__":
    print(f"{RUN_NAME} -- Full Memorization Track")
    print(f"  Training: BeamRider WITH EpisodicLifeEnv (hard failure)")
    print(f"  Eval/Check: Same (EpisodicLifeEnv)")
    print(f"  Original probe (SEED=202): MULTIPLE_SCRIPTS at 10M (single check)")
    print(f"  This run (SEED=206): tracked at every 1M steps")
    print(f"  Comparison run (BEAMRIDER_MULTILIFE, SEED=205): no EpisodicLifeEnv")
    print()
    ...
    memorization_callback = MemorizationCheckCallback(
        ...
        summary_lines=[
            f"BEAMRIDER_BASELINE -- Full Memorization Track (Hard Failure)",
            f"Training: BeamRider WITH EpisodicLifeEnv (hard failure)",
            f"Eval/Check: Same (EpisodicLifeEnv)",
            f"Original (SEED=202): MULTIPLE_SCRIPTS at 10M (single check)",
            f"This run (SEED=206): tracked at every 1M steps",
            f"Compare: BEAMRIDER_MULTILIFE (SEED=205, no lives) -> predicted SINGLE_SCRIPT",
