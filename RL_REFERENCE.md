...
**SUMMARY:** Comprehensive PPO reference for this project. Part 1 = training parameters, Part 2 = observation modes, Part 3 = reward shaping + sticky actions, Part 4 = training metric diagnostics, Part 5 = hardware notes, Part 6 = full experiment history, Part 7 = 45+ lessons learned. **Single most important finding (2026-07-14, strengthened 2026-07-27): No model in this project has ever genuinely generalized.** Every sticky-trained model tested without sticky actions collapsed: PPO_26 (60-pt script), PPO_30b (zero script), PPO_31b (31-pt script), PPO_27 (noise-dependent degenerate). Sticky actions mask memorization with noise; they do not prevent or cure it. This independently confirms Zhang et al. (2018). **New since July 14:** The perception-policy gap is the central problem — NatureCNN CAN track the ball perfectly (1.9px MAE, frozen_ball_tracker.py) but PPO never learns to use those features. Dynamics randomization (Y-perturb via setRAM) sustains det=False diversity but does not prevent argmax memorization. Entropy coefficient is not the lever — every value from 0.006 to 0.10 produces SINGLE_SCRIPT. An env pipeline bug (F-022) and missing resume logic (F-023) were discovered and fixed. The nosticky-verification protocol is the only reliable behavioral test. See FLAWS.md, LOGICAL_AUDIT.md, and COMBINATION_MATRIX.md. **Active experiment:** Experiment 5 — higher perturbation probability (prob=0.25, 0.50) via setRAM() on ALE.

**Eval Record: 147.02 (PPO_27 @ 867M, sticky) | ⚠️ PPO_27 confirmed noise-dependent degenerate without sticky (100% zeros). All sticky-trained models are memorized. Eval score is not a reliable quality metric — see Lesson #23.**
...
| — | **PERCEPTION POC** | NatureCNN supervised ball-tracking (non-RL) | — | **NatureCNN can track the ball to 1.9px MAE.** Frozen CNN from a trained PPO model, with a small MLP head trained by supervised regression on ball position. Proves the perception backbone encodes precise ball position — PPO's failure is in the policy head, not the visual features. The perception-policy gap is the central problem. See `frozen_ball_tracker.py`. |
| PPO_34-43 | Pixel | Custom engine (GymBreakout) experiments — teleports, perturbations, frame skip, dropout | varies | **ALL custom-engine findings invalidated for ALE.** The GymBreakout→ALE transfer gap (L-007) is catastrophic: PPO_35 scored 212 pts on GymBreakout → 2 pts on ALE (99.1% drop). These experiments are documented for reference in EXPERIMENTS.md and archived scripts, but conclusions should not be cited for ALE without replication. |
| PPO_44-54 | Pixel | Return to ALE: `setRAM()` dynamics randomization — ball teleport, paddle teleport, X-mirror, combined X+Y at various probabilities | varies | **Early ALE setRAM batch.** Paddle-bounce teleport (44-46) failed — agents learned to avoid hitting the ball. Mid-flight teleport at 60-80% (47-48) too chaotic. X-mirror at high rates (49-50) also failed. Breakthrough: **cooldown mechanism** — guarantee N frames of clean trajectory between perturbations. X-mirror with cooldown (51-53) showed promise. Combined X+Y at 10% (54) killed. See EXPERIMENTS.md Experiment 4b. |
| PPO_55 | Pixel | ALE + Y-perturb: setRAM(101) ±8px, cooldown=30f, prob=10% | det=False: ~17 best | **Y-perturb baseline.** det=True SINGLE_SCRIPT by 12-16M. det=False 8-15 unique sustained through 48M. Established the argmax-script + policy-entropy pattern on ALE. |
| PPO_57 | Pixel | Y-perturb replicate (seed=57) | det=False: ~24 best | **Confirmed Y-perturb pattern.** Stronger scores than PPO_55 by 48M (best ~24 vs ~17). |
| PPO_58 | Pixel | Y-perturb replicate (seed=58) | det=False: ~24 best | **Confirmed run-to-run variance.** Weaker early peak than 55/57, caught up by 48M. |
| PPO_55a-55e | Pixel | Entropy dose-response: ent_coef ∈ {0.01, 0.02, 0.04, 0.025, 0.10}, from PPO_55 9.6M checkpoint | All: SINGLE_SCRIPT | **Entropy hypothesis FALSIFIED.** Every value from 0.006 (1×) to 0.10 (16.7×) produces SINGLE_SCRIPT. Entropy widens the action distribution but does not shift the mode. At 0.10, det=False scores degrade (avg 6.5 vs 12-14) without preventing memorization. See EXPERIMENTS.md Experiment 4c. |
| PPO_57b | Pixel | Entropy replicate (ent=0.02, from PPO_57 9.6M source) | det=False best: 31 | Strongest single-game det=False score in the project (31 pts). From PPO_57's stronger source weights. |
| PPO_92-95 | Pixel | Higher perturbation probability: prob=0.25, prob=0.50, cooldown=30, ±8px | In progress | **Experiment 5 active.** Testing whether higher perturbation rates break argmax memorization. At 10%, ~2-3 perturbations per game. At 50%, ~8-9. Hypothesis: at some probability threshold, scripts become non-viable because the timed paddle position is wrong too often. |
| RBO_01-02 | Pixel | Revenge Brunch: deep pretraining + sticky, targeting superhuman Breakout via massive compute | In progress | **Separate project.** RBO_02 at ~109M/1B steps as of 2026-07-27. Documented in REVENGE_BRUNCH.md. |
...
25. **Frame-level "is the action moving toward the ball" is not a reliable diagnostic for game outcome — but the pilot data was too small to be conclusive (FLAWS.md F-018).** Tested across a 6-game quick-death cluster (61.3% direction-correct, wide CI due to small sample) vs. a 40-game unfiltered control sample (55.3%). The observed 6-percentage-point gap is consistent with sampling noise given the 6-game pilot sample. This investigation didn't provide enough data to evaluate whether the diagnostic works — a larger sample would be needed to settle the question.
...
41. **The perception-policy gap is the central problem.** NatureCNN CAN track the ball perfectly — a frozen CNN from a trained PPO model, with a small supervised MLP head, achieves 1.9px MAE on ball position regression. The visual features encode precise ball and paddle position. But PPO's policy head never learns to use those features — the optimizer always finds a memorized argmax script because scripts are a viable local optimum in deterministic Breakout. The gap is not in perception (what the agent can see) but in optimization (what the agent chooses to do). See `frozen_ball_tracker.py` and EXPERIMENTS.md Experiment 5 motivation.

42. **Dynamics randomization via setRAM() sustains policy entropy but does not prevent argmax memorization.** Y-perturb (ball Y-axis teleport, ±8px, 30f cooldown, 10% probability) on ALE produces the same pattern seen on the custom engine: det=False maintains healthy score diversity (8-15 unique scores), but det=True collapses to a single memorized argmax script by 12-16M steps. Three independent replicates (PPO_55/57/58) confirm this. The argmax always converges because a timed paddle sequence that works ~90% of the time still has higher expected return than a reactive policy. Higher perturbation probability is the next test (Experiment 5).

43. **Entropy coefficient does not prevent argmax collapse — FALSIFIED.** Every value from ent_coef=0.006 (baseline) through 0.10 (16.7×) produces SINGLE_SCRIPT on det=True inference. At 0.10, det=False scores actually degrade (avg 6.5 vs 12-14) without preventing memorization — the worst of both worlds. Entropy widens the action distribution; it does not shift the mode. The optimizer finds the argmax regardless of how wide the distribution is, because the argmax IS the action with highest expected return when scripts are viable. See EXPERIMENTS.md Experiment 4c.

44. **Perceptual noise is not dynamics randomization — CNNs are built to be robust to pixel-level perturbations.** Observation augmentation (frame shift, optical flow noise, dropout) forces the CNN to see through noise but doesn't prevent memorization — it arrives at the same feature-level representations. What forces reactivity is environmental *dynamics* randomization: if the ball is literally in a different place than a timed script expects, no amount of CNN invariance compensates. This is domain randomization (sim-to-real) applied to the memorization problem. See COMBINATION_MATRIX.md for a full comparison of perceptual vs. dynamics methods.

45. **The cooldown mechanism is critical for perturbation-based training.** Without cooldown (guaranteed N frames of clean trajectory between perturbations), the environment is too chaotic for learning. PPO_47-48 (mid-flight teleport at 60-80%, no cooldown) failed because the policy never experienced normal physics long enough to learn. Cooldown=30 frames at 10% probability means the policy sees clean ball trajectories ~90% of the time while still experiencing 2-3 perturbations per game. The ratio of clean-to-perturbed frames is a key hyperparameter.

46. **Run-to-run variance is real and meaningful — single-replicate conclusions are unreliable.** PPO_55, 57, and 58 — identical Y-perturb configs, different seeds — produced different peak timing, score magnitudes, and diversity levels. PPO_57 peaked higher than PPO_55 which peaked higher than PPO_58. Identical configs do not produce identical results. Every claim about a specific parameter effect should be replicated across at least 2-3 seeds.

47. **The environment pipeline is part of the experimental apparatus — wrapper ordering and presence matters (F-022).** `make_check_env` missing `EpisodicLifeEnv` and `AutoResetWrapper` produced 18+ consecutive false INCOMPLETE det=True verdicts for PPO_55b, which were interpreted as a breakthrough. The MemorizationCheckCallback's autoreset logic requires EpisodicLifeEnv to detect game-over; without it, 0 games complete within max_check_steps. Always verify the check env by inspecting at least one completed check with non-zero games_played before trusting anomalous signals.

48. **Every training script must include resume logic as a standard component (F-023).** Six entropy variant scripts (PPO_55a-55e, 57b) lacked `get_latest_checkpoint()` and resume logic. On every restart, they silently reloaded from the 9.6M source checkpoint, discarding all training progress. With checkpoint save_freq=100,000 (model steps) and 32 envs, a restart at 38.4M that reloaded from 9.6M lost 28.8M steps. The `fresh_start` flag should be printed prominently on startup. A template or base class would prevent this class of bug.

49. **GymBreakout-to-ALE transfer is catastrophic (L-007, CONFIRMED).** PPO_35 cross-evaluated on ALE/Breakout-v5: GymBreakout 212 pts → ALE 2 pts (99.1% drop). The custom engine does not approximate authentic Atari Breakout. All post-Experiment-4 custom-engine conclusions should be treated as custom-engine findings pending ALE replication. Every new experiment must train and evaluate on the same environment.

50. **Never kill an experiment based on wrong-environment data (L-003).** PPO_35 was killed because its memorization track (ALE/Breakout-v5) showed 268 SINGLE_SCRIPT verdicts — but PPO_35 trained on GymBreakout, and the callback tested ALE. The project's own documentation warns this data is "meaningless for GymBreakout-trained models." If a metric comes from the wrong environment, it cannot support a kill decision.
