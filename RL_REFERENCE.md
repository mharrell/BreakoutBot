...
Unlike reward shaping, `repeat_action_probability=0.25` doesn't touch the reward signal at all — it modifies the environment dynamics so 25% of steps repeat the previous action regardless of what the agent chose. This is the Machado et al. (2018) standard mitigation for memorization in deterministic Atari environments. Confirmed effects (PPO_26 vs. PPO_25, matched 10,000-game single-env comparison):
- Improves training stability and general single-env play consistency (higher average and median score)
- **Completely eliminates** the deterministic zero-score launch-angle failure mode (0% vs. 20.0% — see Lesson #22)
- Does **not** reduce funnel exploit completion rate — an earlier hypothesis to that effect was based on a small mismatched-threshold sample and didn't hold up (PPO_26's funnel rate is actually higher than PPO_25's: 0.07% vs 0.02%)
...
| PPO_26 | Pixel | Continued from PPO_25 best_model + `repeat_action_probability=0.25`, 64 envs | 134.16 (eval) | **Completed at 1,001,828,352 total steps.** Eval peak 134.16 @ 905.6M, behind PPO_27 on eval score — but single-env (10,000 games, matched script) tells a different story: avg **54.3**, median 46.0, **0% zero-score games**, funnel rate 0.07% (7/10k) — better than PPO_25 on every single-env metric. See EXPERIMENTS.md Experiment 1 update |
| PPO_27 | Pixel | Fresh agent, `repeat_action_probability=0.25` from step one, 32 envs | **147.02** ✅ | **Current all-time eval record**, set at 867.2M steps. But single-env (10,000 games, matched script): avg **27.9**, median 23.0, **21.3% zero-score games** — worst single-env performance of the three runs, despite highest eval. Sticky-from-scratch underperformed both no-sticky (PPO_25) and inherited-weights+sticky (PPO_26). See EXPERIMENTS.md for analysis |
...
| PPO_27 | 481.6M – 867.2M (completed) | ~104 (full run) | **147.02** @ 867.2M 🏆 | 80.26 @ 640.0M | **Run complete.** Current all-time eval record. Both floor and ceiling rose steadily across quarters (Q1 mean 97→Q4 mean 112; Q1 max 114→Q4 max 147). Single-env (10k games) tells a very different story — see below |
...
| Funnel rate (400+) | 0.02% | **0.07%** | 0.01% |
| Best single-env score | 406 | **415** | 406 |

PPO_26 beats PPO_25 on every single-env metric measured. PPO_27 (the eval record holder) is the **worst** of the three on single-env play — avg 27.9, zero-score 21.3% (higher than PPO_25's 20.0%). The eval-score leaderboard and the single-env leaderboard are not just different rankings — they're nearly inverted. See EXPERIMENTS.md for the full analysis.
...
22. **PPO_25's "zero-score" failure mode requires deep non-sticky foundation + sticky refinement to fix — sticky actions alone are not enough.** PPO_26 (838M non-sticky + ~1B sticky) eliminated zero-score games completely (0 in 10,000). But PPO_27 (867M all-sticky from scratch) has a 21.3% zero-score rate — *worse* than PPO_25's 20.0% no-sticky baseline. The earlier conclusion that "sticky actions fix the zero-score problem" was wrong — it was the combination of deep prior training *and* sticky actions that produced PPO_26's result, not sticky actions alone.
23. **Eval score and single-env score are different leaderboards — and they're nearly inverted.** PPO_27 has the highest eval score (147.02) but the worst single-env performance (avg 27.9, 21.3% zero-score, 0.01% funnel rate). PPO_26 has the best single-env numbers (avg 54.3, 0% zero-score, 0.07% funnel rate) but trails on eval (134.16). The metric that looks better during training (eval) does NOT predict which model will play better in real sequential games. Always verify with 10k-game single-env funnel runs before drawing conclusions about model quality.
...
27. **Fresh sticky-from-scratch training produces the worst single-env performance despite the best eval — inherited weights matter enormously.** PPO_27 (all-sticky, ~867M steps) averaged 27.9 with 21.3% zero-score in single-env play — worse than PPO_25 (no-sticky, ~1B steps) at 34.6 / 20.0%. The zero-score immunity found in PPO_26 required 838M steps of non-sticky foundation *before* sticky actions were introduced. PPO_27's result strongly suggests that sticky actions are a refinement technique for an already-competent policy, not a training regime you should apply from step one.
28. **The proven recipe for single-env quality: deep non-sticky foundation → sticky refinement.** PPO_26 (~838M non-sticky + ~1B sticky) outperforms both PPO_25 (all non-sticky) and PPO_27 (all sticky) on every single-env metric measured. This is the formula to replicate or build on for future experiments.
