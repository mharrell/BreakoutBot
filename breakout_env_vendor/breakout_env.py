...
class Breakout(object):
  def __init__(self, config={}):
    ...
    self.obs_base = np.load(os.path.join(os.path.dirname(__file__), 'assets', 'base.npy'), allow_pickle=True)
    self.digits = [np.load(os.path.join(os.path.dirname(__file__), 'assets', '{}.npy'.format(i)), allow_pickle=True) for i in range(10)]
    self.render_bb = {
