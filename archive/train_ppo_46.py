...
with ball teleportation on paddle bounce (20% probability) to force the
...
Design:
  - Training:  ALE/Breakout-v5 + ALEBreakoutRandomized(teleport_prob=0.30)
               Ball teleports to random (X,Y) on 20% of paddle bounces.
...
def make_training_env():
    """ALE/Breakout-v5 with ball teleportation on 20% of paddle bounces.
...
def make_eval_env():
    ...
    Uses frameskip=4 for speed (eval doesn't need fine control granularity).
    Training remains at frameskip=1.
...
def make_check_env():
    ...
    Uses frameskip=4 so 20 games complete within the 200k-step safety cap
    (at frameskip=1, dead models can hit the cap before finishing one game).
    Training remains at frameskip=1.
