"""
PPO_53 — ALE Experiment 3: X-Mirror Ball (cooldown=60, prob=5%)

Follows PPO_51 (cooldown=30, prob=10%) which was the first model to break
the 0-1 point floor on clean ALE Breakout, reaching SINGLE_SCRIPT scores
of 8-11 points by 24M steps. The trend: lower mirror rates = higher scores.

This version doubles the cooldown to 60 frames (1.0 second) and halves the
post-cooldown probability to 5%. Expected: ~1-2 mirrors per life, each
followed by a full second of guaranteed clean trajectory. The ball follows
natural physics >95% of the time — mirrors are rare interruptions.

Design:
  - Training:  ALE/Breakout-v5 + ALEBreakoutXMirror(cooldown=60, prob=0.05)
               Ball X reflects across center. 60-frame cooldown
               guarantees 1.0s clean trajectory windows between mirrors.
...
MIRROR_COOLDOWN = 60   # frames — guaranteed clean window between mirrors (1.0s)
...
def make_training_env():
    """ALE/Breakout-v5 with X-mirror at 10%.
...
if __name__ == "__main__":
    ...
    print(f"    PPO_51 (cooldown 30, prob 10%): 8-11 pt scripts at 24M")
    ...
    memorization_callback = MemorizationCheckCallback(
        ...
        summary_lines=[
            f"PPO_53 — ALE Experiment 3 (X-mirror, cooldown={MIRROR_COOLDOWN}f, prob={MIRROR_PROB})",
