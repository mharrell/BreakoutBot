"""
PPO_50 — ALE Experiment 3: X-Mirror Ball (80%)

Pairs with PPO_49 (60%). Tests whether extreme X-mirroring forces
horizontal ball-tracking. At 80% per frame, the ball reflects left↔right
on almost every frame — camping on one side is categorically impossible.

Follows PPO_47-48 which tested mid-flight position teleport at 60-80% and
found both models learned degenerate stationary-paddle strategies. Position
teleport scored fake brick points (ball warped into bricks) and made
camping on one side viable.

This experiment switches to X-mirroring: reflect the ball's horizontal
position across the playfield center while preserving its natural velocity
and vertical trajectory. The ball stays in open space — no fake brick
scores. The vertical descent to the paddle is always clean.

At 80% per frame, the ball is essentially always on the move horizontally.
A stationary paddle on one side misses the ball almost every time.

Design:
  - Training:  ALE/Breakout-v5 + ALEBreakoutXMirror(mirror_prob=0.80)
               Ball X reflects across center on 80% of mid-flight frames.
...
def make_training_env():
    """ALE/Breakout-v5 with X-mirror at 80%.
    ...
    env = ALEBreakoutXMirror(env, mirror_prob=MIRROR_PROB)
...
if __name__ == "__main__":
    print(f"{RUN_NAME} — ALE Experiment 3: X-Mirror Ball ({MIRROR_PROB*100:.0f}%)")
    print(f"  Training: ALE/Breakout-v5 + ALEBreakoutXMirror(mirror_prob={MIRROR_PROB})")
    print(f"    Ball X reflects across playfield center on {MIRROR_PROB*100:.0f}% of mid-flight frames")
    ...
    memorization_callback = MemorizationCheckCallback(
        ...
        summary_lines=[
            f"PPO_50 — ALE Experiment 3 (X-mirror at 80%)",
            f"Training: ALE/Breakout-v5 + ALEBreakoutXMirror(mirror_prob={MIRROR_PROB})",
            ...
            f"X-mirror: ball reflected across center — no fake brick scores",
            f"Predecessors PPO_47-48 (position teleport): degenerate stationary strategies",
        ],
    )
