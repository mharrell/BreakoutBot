"""
PPO_55a — MILD ENTROPY CONTINUATION (ent_coef: 0.006 → 0.01, 1.67×)
...
VARIANTS (all from PPO_55 step 10M, all Y-only, cooldown=30, prob=10%):
  PPO_55a: ent_coef = 0.010  (1.67×, mild)    ← THIS SCRIPT
...
ENT_COEF_PRE = 0.006
ENT_COEF_POST = 0.010    # mild — 1.67× increase
...
if __name__ == "__main__":
    ...
    memorization_callback = MemorizationCheckCallback(
        ...
        summary_lines=[
            f"PPO_55a — {label} ENTROPY ({ENT_COEF_PRE}→{ENT_COEF_POST}, {mult:.1f}×) from {SOURCE_RUN} 10M",
