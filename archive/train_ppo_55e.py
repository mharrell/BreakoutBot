"""
PPO_55e — EXTREME ENTROPY CONTINUATION (ent_coef: 0.006 → 0.10, 16.7×)
...
VARIANTS (all from PPO_55 step 10M, all Y-only, cooldown=30, prob=10%):
  ...
  PPO_55a: ent_coef = 0.010  (1.67×, mild)
  PPO_55b: ent_coef = 0.020  (3.33×, moderate)
  PPO_55c: ent_coef = 0.040  (6.67×, aggressive)
  PPO_55d: ent_coef = 0.025  (4.17×, upper probe)
  PPO_55e: ent_coef = 0.100  (16.7×, extreme)    ← THIS SCRIPT

CONTROLS (ent_coef = 0.006 fixed):
  PPO_55, PPO_57, PPO_58 — Y-only replicates

All previous variants (0.01-0.04) collapsed to SINGLE_SCRIPT.
This extreme variant tests the limit: can PPO's optimizer concentrate
the argmax even against 16.7× the normal entropy pressure?
================================================================================
"""
import os
import glob
...
ENT_COEF_PRE = 0.006
ENT_COEF_POST = 0.100    # extreme — 16.7× increase
...
if __name__ == "__main__":
    ...
    label = {0.010: "MILD", 0.020: "MODERATE", 0.025: "UPPER", 0.040: "AGGRESSIVE", 0.100: "EXTREME"}.get(ENT_COEF_POST, "CUSTOM")
    ...
    print(f"  Variants:       55a={0.010} (mild)  55b={0.020} (mod)  55c={0.040} (aggr)  55d={0.025} (upper)  55e={0.100} (extreme)")
    ...
    memorization_callback = MemorizationCheckCallback(
        ...
        summary_lines=[
            f"PPO_55e — {label} ENTROPY ({ENT_COEF_PRE}→{ENT_COEF_POST}, {mult:.1f}×) from {SOURCE_RUN} 9.6M",
            f"Y-Perturb: cooldown={PERTURB_COOLDOWN}f, prob={PERTURB_PROB}, ±{PERTURB_RANGE}px",
            f"Source: PPO_55 @ 9.6M — det=False peak was at 10M: 9 unique, avg 11.6, best 16",
