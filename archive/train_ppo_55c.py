"""
PPO_55c — AGGRESSIVE ENTROPY CONTINUATION (ent_coef: 0.006 → 0.04, 6.67×)
...
VARIANTS (all from PPO_55 step 10M, all Y-only, cooldown=30, prob=10%):
  ...
  PPO_55c: ent_coef = 0.040  (6.67×, aggressive)    ← THIS SCRIPT
...
ENT_COEF_PRE = 0.006
ENT_COEF_POST = 0.040    # aggressive — 6.67× increase
...
if __name__ == "__main__":
    ...
    memorization_callback = MemorizationCheckCallback(
        ...
        summary_lines=[
            f"PPO_55c — {label} ENTROPY ({ENT_COEF_PRE}→{ENT_COEF_POST}, {mult:.1f}×) from {SOURCE_RUN} 10M",
