"""
PPO_51 — ALE Experiment 3: X-Mirror Ball (cooldown=30, prob=10%)

Follows PPO_49-50 which tested X-mirror at 60-80% per-frame and found the
rates were too aggressive — the ball mirrored every 1-2 frames, destroying
the learning signal. Models learned fixed 0-1 point scripts.

This version uses a cooldown mechanism: after each mirror, the ball gets
30 frames (0.5 seconds at 60fps) of guaranteed clean trajectory. Once
cooldown expires, each frame has a 10% chance to mirror. Per life: ~2-4
mirrors, each followed by a half-second trackable approach window.
...
if __name__ == "__main__":
    ...
    memorization_callback = MemorizationCheckCallback(
        ...
        summary_lines=[
            f"PPO_51 — ALE Experiment 3 (X-mirror, cooldown={MIRROR_COOLDOWN}f, prob={MIRROR_PROB})",
