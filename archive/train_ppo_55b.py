"""
PPO_55b — MODERATE ENTROPY CONTINUATION (ent_coef: 0.006 → 0.02, 3.33×)
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    if fresh_start:
        print(f"{RUN_NAME} — FRESH START: {label} ENTROPY ({ENT_COEF_PRE} → {ENT_COEF_POST}, {mult:.1f}×)")
        print(f"  Source:         {SOURCE_RUN} @ {SOURCE_CHECKPOINT_STEP//1_000_000}M steps")
    else:
        print(f"{RUN_NAME} — RESUMING: {label} ENTROPY ({ENT_COEF_POST})")
