"""
PPO_57 — ALE Experiment 4b-REPLICATE: Y-Perturb Only (cooldown=30, prob=10%, ±8px, seed=57)
...
Design:
  - Training:  ALE/Breakout-v5 + ALEBreakoutYPerturb(cooldown=30, prob=0.10, range=8, seed=57)
               Ball Y shifts by ±8px with 30-frame cooldown between perturbations.
               Different perturbation sequence from PPO_55 (seed=57 vs default).
...
Predecessors:
  ...
  PPO_57 (this run): exact replicate with seed=57 — quantifies run-to-run variance
"""
import os
import glob
import cv2
...
SEED = 57               # different from PPO_55's default for run-to-run comparison
...
def make_training_env():
    """ALE/Breakout-v5 with Y-perturb only (seed=57).
...
if __name__ == "__main__":
    print(f"{RUN_NAME} — ALE Experiment 4b-REPLICATE: Y-Perturb Only (seed={SEED})")
    ...
    print(f"    PPO_57 (this run): replicate with seed=57")
    ...
    memorization_callback = MemorizationCheckCallback(
        ...
        summary_lines=[
            f"PPO_57 — ALE Experiment 4b-REPLICATE (Y-perturb only, cooldown={PERTURB_COOLDOWN}f, seed={SEED})",
