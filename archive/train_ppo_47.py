"""
PPO_47 — ALE Experiment 2: Mid-Flight Ball Teleportation (60%)

Follows PPO_44-46 which tested paddle-bounce teleport at 10-20% and found
all three models memorized dead scripts scoring ~0 points across all
checkpoints. Paddle-bounce teleport punishes hitting the ball — the model
learns to ignore the ball rather than track it.

This experiment switches to mid-flight teleportation: the ball is randomized
while traveling through the upper/mid playfield, never near the paddle.
Hitting the ball is never penalized. The final approach is always clean.

At 60% per frame, the ball's position in the upper field is effectively
random — memorized timed trajectories are impossible. The policy MUST
track the ball reactively to succeed.

Design:
  - Training:  ALE/Breakout-v5 + ALEBreakoutFlightRandomized(teleport_prob=0.60)
               Ball teleports mid-flight on 60% of frames in the upper field.
  - Eval:      ALE/Breakout-v5 (clean, no teleport)
...
def make_training_env():
    """ALE/Breakout-v5 with mid-flight ball teleportation at 60%.

    Pipeline: ALE -> NoopResetEnv -> ALEBreakoutFlightRandomized ->
...
def make_eval_env():
    ...
    env = ClipRewardEnv(env)
    env = Monitor(env)
    env = AutoResetWrapper(env)  # autoreset between episodes — prevents hang in evaluate_policy
    return env
...
if __name__ == "__main__":
    print(f"{RUN_NAME} — ALE Experiment 2: Mid-Flight Ball Teleportation (60%)")
    print(f"  Training: ALE/Breakout-v5 + ALEBreakoutFlightRandomized(teleport_prob={TELEPORT_PROB})")
    print(f"    Ball teleports mid-flight on {TELEPORT_PROB*100:.0f}% of frames in upper field")
    print(f"    Never teleports near paddle (Y>={ALEBreakoutFlightRandomized.NO_TELEPORT_Y})")
    print(f"    Hitting the ball is never penalized")
    ...
    print()
    print(f"  Hypothesis: At 60% per frame, ball position in upper field is")
    print(f"  effectively random. Frame-exact memorized trajectories are")
    print(f"  impossible. The policy must track the ball reactively.")
    print(f"  Mid-flight teleport avoids the paddle-bounce penalty problem.")
    print(f"  Predecessors PPO_44-46 (10-20% paddle-bounce): all dead scripts.")
    ...
    memorization_callback = MemorizationCheckCallback(
        ...
        summary_lines=[
            f"PPO_47 — ALE Experiment 2 (mid-flight ball teleport at 60%)",
            f"Training: ALE/Breakout-v5 + ALEBreakoutFlightRandomized(teleport_prob={TELEPORT_PROB})",
            ...
            f"Mid-flight teleport: never penalizes hitting the ball",
            f"Predecessors PPO_44-46 (10-20% paddle-bounce): all dead scripts",
        ],
    )
