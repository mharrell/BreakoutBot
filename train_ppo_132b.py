"""
PPO_132b — Experiment 35b Phase 2: Continue PPO_132a at scale=0.0 (clean Breakout)

PPO_132a trained 15M steps with proximity reward (scale=0.05) to establish
tracking. This phase continues for 10M more steps on CLEAN Breakout with
NO proximity reward. Tests whether tracking behavior persists when the
bonus is removed.

If reactivity survives the switch to clean Breakout, it means the policy
internalized ball-tracking as a general behavior rather than just
maximizing the proximity bonus.

Design:
  - CONTINUATION from ./models/PPO_132a/final_model.zip (15M steps)
  - NO ProximityRewardWrapper — pure Breakout
  - Target: 25M total (10M additional)
...
RESUME_FROM = "./models/PPO_132a/final_model.zip"
...
def make_training_env():
    """Clean Breakout — NO proximity reward."""
...
if __name__ == "__main__":
    print(f"{RUN_NAME} — Experiment 35b Phase 2: Continue at scale=0.0")
    print(f"  Resume from: {RESUME_FROM} (15M steps at scale=0.05)")
    print(f"  Training: Clean ALE/Breakout-v5 — NO proximity reward")
    print(f"  Target: {TARGET_STEPS:,} total steps (10M additional)")
    print(f"  Hypothesis: tracking established in phase 1 persists without bonus")
    print()
    ...
    print(f"Loading PPO_132a from {RESUME_FROM}...")
