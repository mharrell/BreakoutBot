"""
PPO_59 — Experiment 4d: Higher Y-Perturb Probability (prob=0.25, cooldown=30, ±8px)

Dose-response probe: at what perturbation probability do memorized scripts become
non-viable? PPO_55/57/58 established that prob=0.10 produces the argmax-script +
policy-entropy pattern — det=True collapses to SINGLE_SCRIPT, det=False stays
diverse. At 10%, a ~1000-frame game experiences ~3-4 perturbations — the script
works on 90% of frames, which is enough for the argmax to concentrate.

At prob=0.25, the expected perturbations per game rises to ~8-9. The script is
wrong on 1 in 4 eligible cycles. Hypothesis: this crosses the threshold where
timed sequences are too unreliable for the argmax to settle on one.

Part of a two-point dose-response with PPO_60 (prob=0.50).

Design:
  - Training:  ALE/Breakout-v5 + ALEBreakoutYPerturb(cooldown=30, prob=0.25, range=8)
...
Controls:
  ...
  PPO_60 (prob=0.50, this experiment's pair)

Success criterion: det=True MULTIPLE_SCRIPTS (3+ unique scores) at 50M steps.
This would be the first model in project history where the deterministic policy
produces genuinely varied behavior.
"""
import os
import glob
import cv2
...
PERTURB_PROB = 0.25     # 2.5× baseline — target ~8-9 perturbations per game
...
def make_training_env():
    """ALE/Breakout-v5 with Y-perturb at prob=0.25.
...
if __name__ == "__main__":
    print(f"{RUN_NAME} — Experiment 4d: Higher Y-Perturb Probability (dose-response)")
    ...
    print(f"    ~8-9 perturbations per game (2.5× baseline prob=0.10)")
    ...
    print()
    print(f"  Hypothesis: At prob=0.25, scripts are wrong on 1 in 4 eligible cycles.")
    print(f"  The argmax can no longer find a sequence that works reliably enough to")
    print(f"  dominate. If det=True shows MULTIPLE_SCRIPTS, we've found the threshold.")
    print(f"  Controls:")
    print(f"    PPO_55/57/58 (prob=0.10): SINGLE_SCRIPT det=True by 12-16M")
    print(f"    PPO_60 (prob=0.50): paired upper probe")
    ...
    memorization_callback = MemorizationCheckCallback(
        ...
        summary_lines=[
            f"PPO_59 — Experiment 4d (Y-perturb prob=0.25, cooldown={PERTURB_COOLDOWN}f)",
            f"Y-Perturb: cooldown={PERTURB_COOLDOWN}f, prob={PERTURB_PROB}, ±{PERTURB_RANGE}px",
            f"Training: ALE/Breakout-v5 + ALEBreakoutYPerturb (prob=0.25, 2.5× baseline)",
            ...
            f"Dose-response: tests whether prob=0.25 crosses the script-viability threshold",
            f"Controls: PPO_55/57/58 (prob=0.10, all SINGLE_SCRIPT), PPO_60 (prob=0.50)",
