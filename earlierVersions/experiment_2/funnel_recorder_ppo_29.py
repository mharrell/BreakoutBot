...
NUM_GAMES = 1000   # smaller cap — fresh env per game is slower
...
def make_env(seed):
    ...
    ball launch conditions when the VecEnv wrapper chain swallows
    seed() calls before they reach the ALE."""
...
# Load model once — we'll reuse it across fresh envs
...
print(f"Score variety check: how many unique scores? {len(set(scores))}")
