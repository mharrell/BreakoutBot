...
def make_raw_env(brick_addrs=None):
    """Build a raw (non-vec) Breakout env WITHOUT NoopResetEnv.

    NoopResetEnv adds random 0-30 NOOP frames at reset, causing timing offsets
    that confound action divergence: a memorized script indexed by timestep
    produces different actions at different timing offsets, creating false
    action divergence. Removing it means divergence can ONLY come from
    different game states caused by different brick layouts.
    """
    env = gym.make("ALE/Breakout-v5", frameskip=4, repeat_action_probability=0)
    # NO NoopResetEnv — zero timing confound. Both sides start at t=0.
...
if __name__ == "__main__":
    ...
    def _make_dummy():
        ...
        # NO NoopResetEnv — zero timing confound
        e = FireResetEnv(e)
        e = EpisodicLifeEnv(e)
