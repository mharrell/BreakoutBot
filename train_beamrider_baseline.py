"""
BeamRider Baseline Probe — does PPO memorize a script in deterministic BeamRider?

BeamRider is the pure test case for shooters: fully deterministic enemy waves,
fixed level layout, no random UFOs like Space Invaders. If Space Invaders'
partial randomness breaks memorization, BeamRider isolates whether it's the
determinism that matters.

Quick probe: 1 seed, 10M steps, nosticky verification at end.

Design:
  - "ALE/BeamRider-v5", frameskip=4
  - Standard shooter wrappers: NoopResetEnv, FireResetEnv, EpisodicLifeEnv
...
if __name__ == "__main__":
    print(f"{RUN_NAME} — Multi-Env Probe: Deterministic BeamRider")
    print(f"  Hypothesis: fully deterministic shooter -> SINGLE_SCRIPT")
    print(f"  Target: {TARGET_STEPS:,} steps (~2 hours)")
    print(f"  Wrappers: NoopResetEnv, FireResetEnv, EpisodicLifeEnv (standard shooter)")
    print()
    ...
    memorization_callback = MemorizationCheckCallback(
        ...
        summary_lines=[
            f"BEAMRIDER_baseline — Multi-Env Probe",
            f"Env: ALE/BeamRider-v5 (fully deterministic shooter)",
            f"Standard wrappers: NoopResetEnv, FireResetEnv, EpisodicLifeEnv",
            f"Hypothesis: fully deterministic waves -> SINGLE_SCRIPT",
